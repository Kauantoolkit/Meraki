...
export class BidAcceptedConsumer implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.rabbit.subscribe(
      ...
      'bid.accepted',
      async (message) => {
        const { projectId, specialistId, bidId, proposedBudget, milestoneProposals } = message.payload || message;
        this.logger.log(`bid.accepted recebido: project=${projectId} specialist=${specialistId}`);
        await this.assignSpecialist.execute(projectId, specialistId, bidId, proposedBudget, milestoneProposals);
      },
    );
  }
}
