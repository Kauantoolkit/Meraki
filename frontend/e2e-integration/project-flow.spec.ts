...
import { registerAndLogin, createSkillInCatalog } from './helpers/api'

const PROJECT_TITLE = 'Sistema de Agendamento E2E'

/** Seleciona uma skill do catálogo: digita no input, aguarda a sugestão aparecer e clica nela. */
async function selectSkillFromCatalog(page: Parameters<typeof registerAndLogin>[0], skillName: string) {
  await page.getByTestId('cp-skill-input').fill(skillName)
  await page.getByTestId(`cp-skill-suggestion-${skillName.toLowerCase()}`).waitFor({ timeout: 5_000 })
  await page.getByTestId(`cp-skill-suggestion-${skillName.toLowerCase()}`).click()
}
...
test.describe('UC: Criar Projeto (RF03)', () => {
  test('company preenche o formulário e cria projeto com milestones via UI', async ({ page }) => {
    ...
    // Garante que a skill existe no catálogo antes de tentar usá-la
    await createSkillInCatalog(page, 'NestJS')
    ...
    await selectSkillFromCatalog(page, 'nestjs')
  ...
  test('projeto aparece no dashboard após criação', async ({ page }) => {
    ...
    await createSkillInCatalog(page, 'React')
    ...
    // Step 2
    await expect(page.locator('text=Adicionar Tecnologia')).toBeVisible({ timeout: 5_000 })
    await selectSkillFromCatalog(page, 'react')
    await page.getByTestId('cp-next-2').click()
